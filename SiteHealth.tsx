...
import { seedDatabase, getAllCooks, getMenuItemsByLocation } from '../../../data';
...
const SiteHealth: React.FC = () => {
    ...
    const runAudit = useCallback(async () => {
        ...
        try {
            // Cargar cocineros desde Firebase
            const cooksFromDb = await getAllCooks();
            cooksFromDb.forEach((c) => {
                if (c.imageUrl) items.push({ url: c.imageUrl, context: `Chef: ${c.name}` });
            });

            // Cargar menús desde Firebase (BOG y MDE)
            const [bogMenus, mdeMenus] = await Promise.all([
                getMenuItemsByLocation('BOG'),
                getMenuItemsByLocation('MDE'),
            ]);
            [...(bogMenus || []), ...(mdeMenus || [])].forEach((m: MenuItem) => {
                if (m.imageUrl) items.push({ url: m.imageUrl, context: `Plato: ${m.name}` });
            });
        } catch (e) {
            console.error('Error cargando assets para auditoría:', e);
        }
    ...
    const handleSync = async () => {
        if (!window.confirm('¿Estás seguro de que deseas sincronizar todos los datos? Esto sobrescribirá los datos existentes en Firestore.')) return;
            ...
            <div className="flex justify-between items-center mb-10">
                ...
                <div className="flex gap-4">
                    <Button
                        ...
                        disabled={isSyncing}
                        variant="outline"
                        className="shadow-lg uppercase tracking-widest !text-xs !border-green-900 !text-green-900"
                    >
