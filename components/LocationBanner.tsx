...
const LocationBanner: React.FC = () => {
  const { suggestion, setLocationPreference, config } = useGeolocation();
  ...
  const handleSwitch = () => {
    setLocationPreference(suggestion);
    setLocation(suggestion as LocationCode);
  };
    ...
    <div className="bg-hungers-green-950 text-white py-4 px-6 shadow-2xl sticky top-0 z-[110] border-b border-white/10 animate-fade-in">
      <div className="container mx-auto flex flex-col md:flex-row items-center justify-center gap-6">
        ...
        <div className="flex items-center gap-4">
          <Button 
            variant="primary" 
            className="!py-2 !px-6 !text-[11px] !shadow-none font-black uppercase tracking-widest"
            onClick={handleSwitch}
          >
            Sí, cambiar
          </Button>
          <button 
            onClick={() => setLocationPreference('dismissed')}
            className="text-[11px] text-white/40 hover:text-white underline underline-offset-4 transition-colors font-black uppercase tracking-widest"
          >
            Ahora no
          </button>
        </div>
